...
void OnTick() {
   ...
   if(IsVisualMode()) DisplayZonesOnChart();
   ...
   for(int i = 2; i < barsCount - 2; i++) {
      ...
      if(rates[i].high > rates[i-1].high && rates[i].high > rates[i-2].high && 
         ...
         if(!existingZone && zoneCount < MAX_ZONES) {
            ...
            zones[zoneCount].volumeAtFormation = rates[i].tick_volume;
            zoneCount++;
            pivotCount++;
         }
      }
...
void CalculateZoneStrengths() {
   ...
   for(int i = 0; i < zoneCount; i++) {
      ...
      double ageInDays = (currentTime - zones[i].firstTouch) / (60 * 60 * 24);
      double ageFactor = 0;
...
void CheckTradeConditions() {
   // Don't check for new trades if we already have open positions
   ...
   for(int i = 0; i < zoneCount; i++) {
      ...
      if(Trade_EnableBounces && zones[i].strength >= Trade_BouncesMinScore) {
         ENUM_TRADE_SCENARIO bounceScenario = CheckBounceScenario(i, currentPrice, ratesEntry, adx[0], rsi[0]);
         
         if(bounceScenario == SCENARIO_BOUNCE) {
            ExecuteBounceTrade(i, currentPrice, atr[0]);
      ...
      if(Trade_EnableBreakouts && zones[i].strength >= Trade_BreakoutsMinScore) {
         ENUM_TRADE_SCENARIO breakoutScenario = CheckBreakoutScenario(i, currentPrice, ratesEntry, adx[0]);
         
         if(breakoutScenario == SCENARIO_BREAKOUT) {
            ExecuteBreakoutTrade(i, currentPrice, atr[0]);
            return; // Execute only one trade at a time
         }
      }
   }
...
ENUM_TRADE_SCENARIO CheckBreakoutScenario(int zoneIndex, double currentPrice, MqlRates &rates[], double adxValue) {
   ...
   if(volume[1] <= volume[2]) return SCENARIO_NONE; // No volume confirmation
...
void ExecuteBounceTrade(int zoneIndex, double currentPrice, double atr) {
   ...
   if(zones[zoneIndex].isSupport) {
      ...
      stopLoss = zones[zoneIndex].lowerBound - 10 * pipSize; // Below support zone
      takeProfit = currentPrice + (currentPrice - stopLoss) * Trade_RewardRatio; // R:R ratio
      
      // Calculate position size based on risk
      double positionSize = CalculatePositionSize(riskAmount, MathAbs(currentPrice - stopLoss));
      ...
      if(trade.Buy(positionSize, _Symbol, 0, stopLoss, takeProfit, "Bounce_Support_" + zones[zoneIndex].id)) {
         ...
         Print("Error executing bounce trade: ", GetLastError());
      }
   } else {
      // Short trade from resistance
      stopLoss = zones[zoneIndex].upperBound + 10 * pipSize; // Above resistance zone
      takeProfit = currentPrice - (stopLoss - currentPrice) * Trade_RewardRatio; // R:R ratio
...
void ExecuteBreakoutTrade(int zoneIndex, double currentPrice, double atr) {
   ...
   if(zones[zoneIndex].isSupport) {
      ...
      stopLoss = zones[zoneIndex].upperBound + 10 * pipSize; // Above broken support
      takeProfit = currentPrice - (stopLoss - currentPrice) * Trade_RewardRatio; // R:R ratio
      ...
      if(trade.Sell(positionSize, _Symbol, 0, stopLoss, takeProfit, "Breakout_Support_" + zones[zoneIndex].id)) {
         ...
         Print("Error executing breakout trade: ", GetLastError());
      }
   } else {
      // Long trade on resistance breakout
      stopLoss = zones[zoneIndex].lowerBound - 10 * pipSize; // Below broken resistance
      takeProfit = currentPrice + (currentPrice - stopLoss) * Trade_RewardRatio; // R:R ratio
...
double CalculatePositionSize(double riskAmount, double stopLossPips) {
   ...
   double positionSize = riskAmount / (stopLossPips * riskPerPip);
...
void DrawPatternOnChart(PatternResult &pattern, MqlRates &rates[], int startIdx) {
   if(!pattern.detected || !IsVisualMode()) return;
   ...
   if(pattern.patternName.Find("Engulfing") >= 0) {
   ...
   else if(pattern.patternName.Find("Star") >= 0) {
   ...
   else if(pattern.patternName.Find("Doji") >= 0) {
      // Highlight the doji
